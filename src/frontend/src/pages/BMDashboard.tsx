...
export function BMDashboard() {
  ...
  const { data: apps = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      if (actor) {
        try {
          return (await actor.getAllWorkflowApplications(token)).filter(
            (a) => a.status === "BMReview",
          );
        } catch {}
      }
      return getLocalApps().filter((a) => a.status === "BMReview");
    },
  ...
  const actionMutation = useMutation({
    mutationFn: async ({
      ...
      if (actor) {
        try {
          if (action === "approve") await actor.bmApprove(appId, token, remark);
          else await actor.bmReject(appId, token, remark);
          return;
        } catch {}
      }
      const raw = localStorage.getItem("loanApplications") || "[]";
      ...
      if (idx >= 0) {
        arr[idx].status = action === "approve" ? "AdminApproval" : "Rejected";
        localStorage.setItem("loanApplications", JSON.stringify(arr));
      }
    },
    onSuccess: () => {
      toast.success("Decision recorded");
      qc.invalidateQueries({ queryKey: ["bm-apps"] });
      setRemarkApp(null);
    },
    ...
    <div className="flex h-screen bg-gray-50">
      <aside className="w-64 bg-[#0a1628] flex flex-col shrink-0">
        <div className="p-5 border-b border-white/10">
          ...
          />
          <div className="text-white/80 text-xs mt-2">Branch Manager</div>
        </div>
        <nav className="p-4 space-y-1 flex-1">
          ...
          >
            <LayoutDashboard className="h-4 w-4" /> Review Queue
          </button>
        </nav>
        <div className="p-4 border-t border-white/10">
          <div className="text-white/60 text-xs mb-1">Branch Manager</div>
          <div className="text-white text-sm font-medium">
            {localStorage.getItem("adminEmail") || "BM"}
          </div>
          <button
            type="button"
            onClick={() => {
        ...
        <header className="bg-white border-b px-6 py-4 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Applications forwarded by CRM for your decision
            </p>
          </div>
        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="grid grid-cols-3 gap-4 mb-6">
            {[
              ...
              { label: "Reviewed Today", value: 0, color: "bg-green-500" },
              {
          ...
          <div className="bg-white rounded-xl border overflow-hidden">
            <div className="px-5 py-4 border-b">
              <h2 className="font-semibold text-gray-900">
                Pending BM Review ({filtered.length})
              </h2>
            </div>
            {isLoading ? (
